...
public class BookTest3 {
    ...
    public static void main(String[] args) {
        ...
        List<BookVO> returnList= filterTitleBook(list,"title","Resmiserable");
        ...
        List<BookVO> resultList= filterTitleBook(list,"publisher","yolin");

        System.out.println("publisher 로 구분");
    ...
    public static List<BookVO> filterTitleBook(List<BookVO> invertory, String category,String condition){
        List<BookVO> result = new ArrayList<>();

        if("title".equals(category)){
            for(BookVO book : invertory){
                if(condition.equals(book.getTitle())){
                    result.add(book);
                }
            }
        }else if("publisher".equals(category)){
            for(BookVO book : invertory){
                if(condition.equals(book.getPublisher())){
                    result.add(book);
                }
            }
        }
